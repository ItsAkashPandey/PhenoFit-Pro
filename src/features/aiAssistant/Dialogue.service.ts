...
export class DialogueService {
  ...
  setCurrentIntent(intent: IntentType, entities: any[]): { needsClarification: boolean; missingSlots: string[] } {
    console.log('DialogueService.setCurrentIntent called with:', { intent, entities });
    ...
    console.log('Required slots for', intent, ':', requiredSlots);
    
    const providedSlots = this.extractSlots(entities);
    console.log('Provided slots:', providedSlots);
    
    // First, initialize all required slots in unfilled_slots (if not already there)
    ...
    // Now fill all provided slots
    for (const [slotName, value] of Object.entries(providedSlots)) {
      const filled = this.fillSlot(slotName, value);
      console.log('Filled slot', slotName, 'with value', value, ':', filled);
    }
    ...
    console.log('Missing slots after processing:', missingSlots);
    console.log('Final context unfilled_slots:', this.context.unfilled_slots);
  ...
  // Get current filled slots (alias for getFilledSlots)
  getCurrentSlots(): { [key: string]: any } {
    return this.getFilledSlots();
  }

  // Set last action result (for compatibility)
  setLastActionResult(result: any): void {
    // Store in context for reference
    if (this.context.app_state_snapshot) {
      this.context.app_state_snapshot.analysisResults = result;
    }
  }

  // Get conversation summary (for LLM context)
  getConversationSummary(): string {
    const recentMessages = this.context.conversation_history.slice(-5);
    return recentMessages.map(msg => `${msg.role}: ${msg.content}`).join('\n');
  }

  // Check if context should be preserved for follow-up commands
  shouldPreserveContext(intent: IntentType): boolean {
    // Preserve context for styling commands to allow follow-up adjustments
    const preserveContextIntents = ['STYLE_CHART_ELEMENT', 'chart_styling'];
    return preserveContextIntents.includes(intent);
  }

  // Clear current intent with option to preserve for follow-ups
  clearCurrentIntent(force: boolean = false): void {
    if (!force && this.context.current_intent && this.shouldPreserveContext(this.context.current_intent)) {
      console.log('Preserving context for potential follow-up to:', this.context.current_intent);
      // Don't clear immediately - preserve for potential follow-up commands
      return;
    }
    
    this.context.current_intent = null;
    this.context.unfilled_slots = [];
    this.dialogueState.currentIntent = undefined;
  }

  // Generate clarification question for missing slots
  generateClarificationQuestion(missingSlots: string[]): string {
    if (missingSlots.length === 0) {
      return "I have all the information I need.";
    }

    const slotDescriptions: { [key: string]: string } = {
      'chart_element': 'What part of the chart would you like to modify?',
      'styling_property': 'What property would you like to change?',
      'property_value': 'What value would you like to use?',
      'axis_name': 'Which axis? (x or y)',
      'confirmation': 'Should I proceed with this change?'
    };

    const firstMissing = missingSlots[0];
    return slotDescriptions[firstMissing] || `Please specify ${firstMissing}.`;
  }
  ...
  private getRequiredSlotsForIntent(intent: IntentType): Array<{entity: string, description?: string}> {
    const slotDefinitions: { [key: string]: Array<{entity: string, description?: string}> } = {
      // Original intent names (for backward compatibility)
      'style_chart': [
      ...
      // Mapped action intent names (these are what actually get used)
      'STYLE_CHART_ELEMENT': [
        { entity: 'chart_element', description: 'What part of the chart do you want to style?' },
        { entity: 'styling_property', description: 'What property do you want to change?' },
        { entity: 'property_value', description: 'What value do you want to use?' }
      ],
      'SET_CURVE_MODEL': [
        { entity: 'model_type', description: 'What type of curve model?' }
      ],
      'ADJUST_MODEL_PARAMETER': [
        { entity: 'parameter_name', description: 'Which parameter?' },
        { entity: 'parameter_value', description: 'What value?' }
      ],
