import { ChartStyles, ApiService, ModelConfig, HeadersInit } from '../types';
...
1.  **PLOT**: When the user wants to plot data. The payload should contain the x and y columns.
    ...
    - Example: "Plot NDVI vs DAS" -> { "actions": [{ "action": "PLOT", "payload": { "y_column": "NDVI", "x_column": "DAS" }, "confidence": 0.9, "response": "OK, plotting NDVI against DAS." }], "response": "OK, plotting NDVI against DAS." }
...
2.  **STYLE**: When the user wants to change the visual style of a chart element.
    ...
    - Example: "change the background to light blue with 20% opacity" -> { "actions": [{ "action": "STYLE", "payload": { "target": "chartBackground", "properties": { "color": "#ADD8E6", "opacity": 0.2 } }, "confidence": 0.95, "response": "OK, I've set the chart background to light blue with 20% opacity." }], "response": "OK, I've set the chart background to light blue with 20% opacity." }
    - Example: "set line thickness to 3" -> { "actions": [{ "action": "STYLE", "payload": { "target": "fitted", "properties": { "lineWidth": 3 } }, "confidence": 0.9, "response": "OK, I've set the line thickness to 3." }], "response": "OK, I've set the line thickness to 3." }
...
3.  **SET_AXIS**: When the user wants to set the min or max limits of an axis.
    ...
    - Example: "set x axis limits from 0 to 150" -> { "actions": [{ "action": "SET_AXIS", "payload": { "axis": "x", "min": 0, "max": 150 }, "confidence": 0.98, "response": "OK, X-axis limits are now 0 to 150." }], "response": "OK, X-axis limits are now 0 to 150." }
...
4.  **TOGGLE_VISIBILITY**: To show or hide a chart element.
    ...
    - Example: "hide the legend" -> { "actions": [{ "action": "TOGGLE_VISIBILITY", "payload": { "element": "legend", "visible": false }, "confidence": 0.99, "response": "OK, the legend is now hidden." }], "response": "OK, the legend is now hidden." }

5.  **OPTIMIZE**: To run the curve-fitting optimization process.
    - Keywords: optimize, run optimization, fit the curve
    - Example: "optimize the parameters" -> { "actions": [{ "action": "OPTIMIZE", "payload": {}, "confidence": 0.98, "response": "OK, starting parameter optimization." }], "response": "OK, starting parameter optimization." }
...
6.  **SET_LABEL**: To change the text label of an axis.
    ...
    - Example: "change x-axis label to Days" -> { "actions": [{ "action": "SET_LABEL", "payload": { "axis": "x", "text": "Days" }, "confidence": 0.97, "response": "OK, the X-axis label is now 'Days'." }], "response": "OK, the X-axis label is now 'Days'." }

7.  **REVERT**: To undo the last change made by the assistant.
    - Keywords: revert, undo, go back, previous
    - Example: "revert that" -> { "actions": [{ "action": "REVERT", "payload": {}, "confidence": 0.99, "response": "OK, I have reverted the last change." }], "response": "OK, I have reverted the last change." }
...
8.  **MOVE_ELEMENT**: To move a chart element to a specific position.
    ...
    - Example: "move legend to top right" -> { "actions": [{ "action": "MOVE_ELEMENT", "payload": { "element": "legend", "position": "top right" }, "confidence": 0.98, "response": "OK, I've moved the legend to the top right." }], "response": "OK, I've moved the legend to the top right." }
...
9.  **SET_PARAMETER**: To set the value of a model parameter.
    ...
    - Example: "set baseline to 0.2" -> { "actions": [{ "action": "SET_PARAMETER", "payload": { "parameter": "baseline", "value": 0.2 }, "confidence": 0.99, "response": "OK, I've set the baseline to 0.2." }], "response": "OK, I've set the baseline to 0.2." }
...
10. **TOGGLE_PARAM_LOCK**: To lock or unlock a model parameter.
    ...
    - Example: "lock amplitude" -> { "actions": [{ "action": "TOGGLE_PARAM_LOCK", "payload": { "parameter": "amplitude", "locked": true }, "confidence": 0.99, "response": "OK, I've locked the amplitude parameter." }], "response": "OK, I've locked the amplitude parameter." }

11. **UNKNOWN**: If the command is ambiguous or doesn't fit any other category.
    - Example: "hello there" -> { "actions": [{ "action": "UNKNOWN", "payload": {}, "confidence": 0.9, "response": "Sorry, I can only help with charting. You can ask me to 'plot NDVI vs Date' or 'change the background color to blue'." }], "response": "Sorry, I can only help with charting. You can ask me to 'plot NDVI vs Date' or 'change the background color to blue'." }
...
export const parseCommand = async (command: string, columns: string[], styles: ChartStyles, modelConfig: ModelConfig): Promise<NLUIntent> => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { actions: [], response: userMessage };
        }
        ...
        return { actions: [], response: "An error occurred while processing your command." };
    }
};